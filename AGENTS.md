...
- Use native vertical `scroll-snap-type: y proximity` only above 700 px and outside reduced-motion, menu, and modal states. Keep it gentle: the first and last sections do not snap, and settled content has a small top offset. Never replace native scrolling with JavaScript auto-scroll or mandatory snapping.
